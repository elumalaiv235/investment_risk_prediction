...
interface BadgeProps {
  ...
  variant?: 'primary' | 'secondary' | 'success' | 'warning' | 'danger' | 'info' | 'risk';
...
export const Badge: React.FC<BadgeProps> = ({
  ...
  if (variant === 'risk' || riskCategory) {
    const meta = getCategoryMeta(riskCategory || String(children));
    return (
      <span
        className={`inline-flex items-center gap-1.5 rounded-full border ${sizeStyles[size]} ${meta.bgColor} ${meta.badgeText} ${meta.borderColor} ${className}`}
      >
  ...
  const variantStyles = {
    primary: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800',
